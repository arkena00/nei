...
namespace nei
{
    ...
    *         reading 32 bits from a uint8_t will work (by extracting the first 8 bits values and appending 24 zeros afterneath).
    *
    * \see AppendBits
    * \see Read
    */
    ...
    */
    ...
    typename bitset<Block, Allocator>::PointerSequence bitset<Block, Allocator>::Write(const PointerSequence& sequence, std::size_t bitCount)
    {
        ...
        const uint8_t* u8Ptr = static_cast<const uint8_t*>(sequence.first);
        const uint8_t* endPtr = u8Ptr + ((totalBitCount != 0) ? (totalBitCount - 1) / 8 : 0);
        const uint8_t* nextPtr = endPtr + ((totalBitCount % 8 != 0) ? 0 : 1);

        // Read the first block apart to apply a mask on the first byte if necessary
        if (sequence.second != 0)
        {
            uint8_t mask = ~((1U << sequence.second) - 1U);
    ...
    */
    ...
    template<typename Block, class Allocator>
    void bitset<Block, Allocator>::unbounded_set(std::size_t bit, bool val)
    {
        if (bit < m_bitCount)
            set(bit, val);
        else if (val)
    ...
    */
    ...
    template<typename Block, class Allocator>
    bitset<Block, Allocator>& bitset<Block, Allocator>::operator=(const std::string& bits)
    {
        bitset bitset(bits);
